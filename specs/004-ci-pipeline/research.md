...
**Rationale**: FR-010 explicitly allows either "folded in" or "left separate," and per spec.md's Assumptions this was resolved toward "leave separate" — it triggers on a different condition (`paths: charts/spechub/**`, not every PR) and packages/pushes a Helm chart, not an application image; folding it in would add branching complexity to `ci.yml` for a workflow whose trigger condition and payload are both unrelated to the code-correctness gate this feature is building. No stated acceptance criterion depends on merging them.
