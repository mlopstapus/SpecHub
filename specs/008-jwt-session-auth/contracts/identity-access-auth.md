...
Looks up every user matching the (lowercased) email **across all organizations** — email is only unique per-organization (PDR-003), so login cannot scope the lookup to one org it doesn't know yet (research.md §8) — then verifies the password via `bcryptjs.compare` only against the active candidates. Returns `null` (never throws) if no active candidate's password matches — covering unknown email, wrong password, and deactivated account uniformly, per FR-002/FR-008's non-enumeration requirement. On success, signs a JWT (`sub`, `role`, `exp` — `context/auth-conventions.md`), builds the session cookie descriptor, writes a `user.login` audit event (or `user.login_failed` on the rejected path — see below), and returns the `UserSummary` + cookie.
