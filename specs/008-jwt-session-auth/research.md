...
## 8. Login lookup across organizations, not scoped to one

**Decision**: `login(db, email, password)` looks up **every** user row matching the (lowercased) email across all organizations via a new `users-repo.findByEmail(tx, email)` (no `organizationId` filter — login has no org context yet), then bcrypt-compares the password only against the *active* candidates. On a password match, that candidate's organization is the login's organization. On no match: if exactly one candidate existed (active or deactivated), the failed-login audit event attributes to that specific user/org (FR-011's "the matching user, if the email resolves to a real account" — a real, if deactivated, account is still a real account); if zero or more than one candidate existed with no password match, the audit event is unattributed (`actorUserId`/`organizationId` both `null`), identical to a genuinely unknown email — there is no way to know which of several same-email accounts across different organizations a failed attempt "was for."

**Rationale**: `007-user-accounts-registration` deliberately made `email` unique per-`(organization_id, email)`, not globally (PDR-003's whole point — the exact bug of assuming a single global user directory) — so a login flow that queries `WHERE email = ? AND organization_id = ?` cannot exist, because at the point of login the organization isn't known yet; the user hasn't been resolved. This was not addressed by spec.md, data-model.md, or contracts.md's original text (each implicitly assumed "look up the user by email" resolves to a single row), and was only caught during implementation — this is the fix, applied at the lowest layer that actually has the problem (the repo query), not layered on top with a workaround.

**Alternatives considered**:
- Require an `organizationSlug`/`orgId` parameter on `login()`, resolved by a prior UI step (e.g. a subdomain or an org-picker page) — rejected: no such UX exists anywhere in this codebase yet (`context/auth-conventions.md` explicitly notes no subdomain-per-tenant pattern exists), and inventing one is a UI/routing decision far outside this backend-only feature's scope; would also block self-hosted single-org deployments (the common case) with an unnecessary extra step.
- Treat any email collision across organizations as a hard error at write-time (reject creating a second account with an email that exists in another org) — rejected: directly undoes PDR-003's fix, which explicitly exists to *allow* two different organizations to each have `admin@example.com`.
- On multiple candidates with no password match, arbitrarily attribute the failed-login audit event to the first candidate found — rejected: attributing a failed attempt to a specific organization/user when it's genuinely ambiguous which one was targeted would be a false audit record, worse than an honest "unattributed."
