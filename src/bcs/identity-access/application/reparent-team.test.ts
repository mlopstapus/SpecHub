...
describe("reparentTeam", () => {
  ...
  it("rejects reparenting under a team from a different organization, no row changes", async () => {
    ...
    const teamA = await testDb.appDb.transaction((tx) =>
      createTeam(tx, { organizationId: orgA.id, name: "Team A", slug: "team-a" }),
    );
    const teamB = await testDb.appDb.transaction((tx) =>
      createTeam(tx, { organizationId: orgB.id, name: "Team B", slug: "team-b" }),
    );

    await expect(
      testDb.appDb.transaction((tx) => reparentTeam(tx, teamA.id, teamB.id)),
    ).rejects.toThrow(CrossOrgReparentError);

    const [row] = await testDb.appDb.select().from(teams).where(eq(teams.id, teamA.id));
  ...
  it("rejects a reparent that would create a cycle, no row changes", async () => {
    ...
    const x = await testDb.appDb.transaction((tx) =>
      createTeam(tx, { organizationId: org.id, name: "X", slug: "x" }),
    );
    const y = await testDb.appDb.transaction((tx) =>
    ...
    await expect(
      testDb.appDb.transaction((tx) => reparentTeam(tx, x.id, y.id)),
    ).rejects.toThrow(CycleError);

    const [row] = await testDb.appDb.select().from(teams).where(eq(teams.id, x.id));
    expect(row?.parentTeamId).toBeNull();
  });
  ...
  it("a valid reparent succeeds and getTeamChain reflects the new lineage immediately", async () => {
    ...
    const oldParent = await testDb.appDb.transaction((tx) =>
      createTeam(tx, { organizationId: org.id, name: "Old Parent", slug: "old-parent" }),
    );
    const newParent = await testDb.appDb.transaction((tx) =>
    ...
    await testDb.appDb.transaction((tx) => reparentTeam(tx, child.id, newParent.id));

    const chain = await getTeamChain(testDb.appDb, child.id);
  ...
  it("reparenting to the team's own current parent succeeds as a no-op", async () => {
    ...
    await expect(
      testDb.appDb.transaction((tx) => reparentTeam(tx, child.id, parent.id)),
  ...
  it("under two concurrent reparents that would jointly create a cycle, exactly one succeeds", async () => {
    ...
    const a = await testDb.appDb.transaction((tx) =>
      createTeam(tx, { organizationId: org.id, name: "A", slug: "a" }),
    );
    const b = await testDb.appDb.transaction((tx) =>
      createTeam(tx, { organizationId: org.id, name: "B", slug: "b" }),
    );

    const results = await Promise.allSettled([
      testDb.appDb.transaction((tx) => reparentTeam(tx, a.id, b.id)),
      testDb.appDb.transaction((tx) => reparentTeam(tx, b.id, a.id)),
    ]);
